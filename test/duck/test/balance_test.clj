...
(let [date-old (format/parse utility/date-formatter "01/01/1850")
  ...
  (fact (persistence/wait-commit-transaction []) => [])
  (fact (count (persistence/wait-commit-transaction [{:date date-old}])) => 0)
  (fact (count (persistence/wait-commit-transaction [{:date date-old}
                                                     {:date date-past}
                                                     {:date date-future}])) => 1))
...
(let [date-future-1 (format/parse utility/date-formatter "01/12/2019")
  ...
  (fact (persistence/updated-balance (bigdec 0.0) []) => (bigdec 0.0))
  (fact (persistence/updated-balance (bigdec 115000.0) []) => (bigdec 115000.0))

  (fact (persistence/updated-balance (bigdec 25.5) [{:date date-future-1
                                                     :amount (bigdec 2.0)}
                                                    {:date date-future-1
                                                     :amount (bigdec 1.0)}]) => (bigdec 28.5))

  (fact (persistence/updated-balance (bigdec 123.3) [{:date date-old-1
                                                      :amount (bigdec 20.0)}
                                                     {:date date-old-1
                                                      :amount (bigdec -1.0)}]) => (bigdec 142.3)))

;(println (persistence/balance-verification (:amount (bigdec 2.0))))
